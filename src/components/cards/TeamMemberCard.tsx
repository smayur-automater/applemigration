...
export function TeamMemberCard({
  ...
  return (
    <article className="flex flex-col rounded-xl bg-white p-8 shadow-sm">
      <div
        aria-hidden="true"
        className="mb-6 flex aspect-square w-full items-center justify-center rounded-xl bg-slate-900"
      >
      ...
      {credentials && (
        <span className="mt-2 self-start rounded bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">
          {credentials}
        </span>
      )}
      <p className="mt-3 flex-1 text-sm text-slate-500">{bio}</p>
      {linkedIn && (
        <a
          href={linkedIn}
          target="_blank"
          rel="noopener noreferrer"
          className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-slate-900 hover:text-accent-hover transition-colors duration-(--duration-fast)"
        >
