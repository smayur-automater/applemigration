  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--color-slate-600)', opacity: 0.6 }}>
        ...
        <span>/</span>
        <span style={{ opacity: 1, color: 'var(--color-slate-900)' }}>{client.name}</span>
      </div>
      ...
      <div className="bg-white rounded-xl p-6" style={{ boxShadow: 'var(--shadow-sm)' }}>
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h2 className="text-xl font-bold" style={{ color: 'var(--color-slate-900)' }}>{client.name}</h2>
                <p className="text-sm" style={{ color: 'var(--color-slate-600)', opacity: 0.6 }}>{client.email}</p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t" style={{ borderColor: 'var(--color-border)' }}>
          {[
            ...
            <div key={item.label}>
              <p className="text-xs font-medium uppercase tracking-wider mb-1" style={{ color: 'var(--color-slate-600)', opacity: 0.5 }}>
                {item.label}
              </p>
